...
create type house_types as enum ('House','Apartment');
create type family_statuses as enum ('Single','Married','Divorced','Widow','Separated','Other');
...
alter table agencies add constraint check_phone_number check (phone_number like '+996%' );
...
select h.house_type,first_name  from owners o left join rent_infos r on o.id = r.owner_id
                                              left join houses h on r.house_id = h.id order by length(first_name) desc limit 5;
select * from owners;
-- - 2. Уйлордун баалары 1500, 2000 дин аралыгында бар болсо true чыгар, жок болсо false чыгар.
select house_type,houses.furniture from houses where price between 1500 and 2000;
-- - 3. id_лери 5, 6, 7, 8, 9, 10 го барабар болгон адресстерди жана ал адрессте кайсы уйлор бар экенин чыгар.
select a.city,house_type from addresses a inner join houses h on a.id = h.address_id where a.id in (5,6,7,8,9,10) ;
-- - 4. Бардык уйлорду, уйдун ээсинин атын, клиенттин атын, агенттин атын чыгар.
select h.house_type,o.first_name,c.first_name,a.agency_name from rent_infos r inner join owners o on r.owner_id = o.id
...
select h.house_type,h.rating,o.first_name from rent_infos r inner join houses h on r.house_id = h.id
                                                            inner join owners o on r.owner_id = o.id order by rating;
select h.house_type,h.rating,o.first_name from rent_infos r inner join houses h on r.house_id = h.id
                                                            inner join owners o on r.owner_id = o.id order by rating desc ;
-- - 7. Уйлордун арасынан квартиралардын (apartment) санын жана алардын баасынын суммасын чыгар.
select price,sum(price) from houses where house_type='Apartment'group by price;
...
select a.agency_name,a2.city,h.house_type,o.first_name from rent_infos r inner join agencies a on r.agency_id = a.id
                                                                         inner join houses h on r.house_id = h.id
                                                                         inner join addresses a2 on a2.id = h.address_id
                                                                         inner join owners o on r.owner_id = o.id where agency_name = 'My House';
select * from agencies ;
-- - 9. Уйлордун арасынан мебели бар уйлорду, уйдун ээсин жана уйдун адрессин чыгар.
select o.first_name,a2.city,h.house_type from owners o inner join rent_infos ri on o.id = ri.owner_id
                                                       inner join agencies a on ri.agency_id = a.id
                                                       inner join addresses a2 on a.address_id = a2.id inner join houses h on a2.id = h.address_id where furniture is not null ;
-- - 10.Кленти жок уйлордун баарын жана анын адрессин жана ал уйлор кайсыл агентсвога тийешелуу экенин чыгар.
select h.house_type from rent_infos r inner join agencies a on r.agency_id = a.id inner join addresses a2 on a.address_id = a2.id
                                      inner join owners o on r.owner_id = o.id inner join houses h on a2.id = h.address_id where customer_id is null ;
-- - 11.Клиенттердин улутуна карап, улутуну жана ал улуутта канча клиент жашайт санын чыгар.
select count(c.id),nationality from customers c inner join rent_infos ri on c.id = ri.customer_id group by nationality;
-- - 12.Уйлордун арасынан рейтингтери чон, кичине, орточо болгон 3 уйду чыгар.
select houses.house_type,price from houses order by price  and price>avg(price) limit 1;
-- - 13.Уйлору жок киленттерди, клиенттери жок уйлорду чыгар.
select customers.first_name,h.house_type from customers inner join rent_infos ri on customers.id = ri.customer_id
                                                        inner join houses h on ri.house_id = h.id where h.id is   null and customers.id is  null ;
-- - 14.Уйлордун бааларынын орточо суммасын чыгар.
select round(avg(houses.price)) from houses ;
-- - 15.‘A’ тамга менен башталган уйдун ээсинин аттарын, клиенттердин аттарын чыгар.
select o.first_name,c.first_name from rent_infos ri inner join owners o on ri.owner_id = o.id
                                                    inner join customers c on ri.customer_id = c.id where o.first_name like 'A%';
-- - 16.Эн уйу коп owner_ди жана анын уйлорунун санын чыгар.
select o.first_name,count(h.id) from owners  o inner join rent_infos ri on o.id = ri.owner_id
                                               inner join houses h on h.id = ri.house_id group by o.first_name order by count(o.id) desc limit 1;
-- - 17.Улуту Kyrgyzstan уй-булолу customerлерди чыгарыныз.
select * from customers c where nationality = 'Kyrgyz';
select c.nationality from customers c;
-- - 18.Эн коп болмолуу уйду жана анын адресин ал уй кайсыл ownerге таандык ошону чыгарыныз.
select count(h.room),h.house_type,a.city,o.first_name from houses h inner join rent_infos ri on h.id = ri.house_id
                                                                    inner join owners o on ri.owner_id = o.id
                                                                    inner join addresses a on a.id = h.address_id group by h.house_type,first_name,city order by count(h.room) desc limit  1;
-- - 19.Бишкекте жайгашкан уйлорду жана клиентерин кошо чыгарыныз.
select h.house_type,c.first_name,city from rent_infos ri inner join houses h on ri.house_id = h.id
                                                         inner join  customers c on ri.customer_id = c.id inner join addresses a on h.address_id = a.id where city = 'Bishkek';
-- - 20.Жендерине карап группировка кылыныз.
select owners.gender,owners.first_name from owners group by gender,first_name;
-- - 21.Эн коп моонотко ижарага алынган уйду чыгарыныз.
select houses.house_type from houses
